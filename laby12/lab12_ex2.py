...
cwa = cwb = 0 #angular acceleration
g = 9.8
...
while True:
    rate(800)
    caa = (-g / cL * (2 * sin(calpha) - sin(cbeta) * cos(calpha - cbeta)) - cwa ** 2 * sin(
        2 * calpha - 2 * cbeta) / 2 - cwb * cwb * sin(calpha - cbeta)) / (1 + sin(calpha - cbeta) ** 2)
    cab = (-g / cL * (2 * sin(cbeta) - 2 * sin(calpha) * cos(calpha - cbeta)) + cwb * cwb * sin(
        2 * calpha - 2 * cbeta) / 2 + 2 * cwa * cwa * sin(calpha - cbeta)) / (1 + sin(calpha - cbeta) ** 2)
    cwa = cwa+caa*dt
    cwb = cwb+cab*dt
    calpha += cwa * dt
    cbeta += cwb * dt
    cball_1.pos=vector(cL * sin(calpha), -cL * cos(calpha), 0)
    cball_2.pos=vector(cL * (sin(calpha) + sin(cbeta)), -cL * (cos(calpha) + cos(cbeta)), 0)
